...
def get_model():
    ...
    a=tf.keras.layers.Concatenate()([area_e,ts_code_e,industry_e])
...
def train(ts_code="000001.SZ"):
    ...
    model.fit({'lstm_input': t, 'ts_code_input': t1, 'area_input': a, 'industry_input': i}, {"out": r},batch_size=1000,epochs=10)
    save_model(model)
...
def test1():
    ...
    i = []
    r = []
    for line in open(r"D:\data\share\data\split_datas_1", encoding="utf-8"):
        item = json.loads(line)
        t.append(item[0])
    ...
    model.save_weights(r"D:\data\share\model\model_0921")
